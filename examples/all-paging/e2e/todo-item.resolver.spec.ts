...
import { cursorPageInfoField, edgeNodes, offsetConnection, subTaskFields, tagFields, todoItemFields } from './graphql-fragments'
    ...
    describe('PagingStrategies.OFFSET', () => {
      it(`should return a connection`, () =>
        request(app.getHttpServer())
          ...
          .expect(200, {
            data: { todoItemsOffset: { nodes: todoItems, pageInfo: { hasNextPage: false, hasPreviousPage: false } } }
          }))
      ...
      it(`should allow querying`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          todoItemsOffset(filter: { id: { in: [1, 2, 3] } }) {
            ${offsetConnection(todoItemFields)}
          }
        }`
          })
          .expect(200, {
            data: {
              todoItemsOffset: { nodes: todoItems.slice(0, 3), pageInfo: { hasNextPage: false, hasPreviousPage: false } }
            }
          }))
      ...
      it(`should allow sorting`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          todoItemsOffset(sorting: [{field: id, direction: DESC}]) {
            ${offsetConnection(todoItemFields)}
          }
        }`
          })
          .expect(200, {
            data: {
              todoItemsOffset: { nodes: todoItems.slice().reverse(), pageInfo: { hasNextPage: false, hasPreviousPage: false } }
            }
          }))
        ...
        it(`should allow paging with the 'limit' field`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          todoItemsOffset(paging: {limit: 2}) {
            ${offsetConnection(todoItemFields)}
          }
        }`
            })
            .expect(200, {
              data: {
                todoItemsOffset: { nodes: todoItems.slice(0, 2), pageInfo: { hasNextPage: true, hasPreviousPage: false } }
              }
            }))
        ...
        it(`should allow paging with the 'limit' field and 'offset'`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          todoItemsOffset(paging: {limit: 2, offset: 2}) {
            ${offsetConnection(todoItemFields)}
          }
        }`
            })
            .expect(200, {
              data: {
                todoItemsOffset: { nodes: todoItems.slice(2, 4), pageInfo: { hasNextPage: true, hasPreviousPage: true } }
              }
            }))
      })
    })
    describe('PagingStrategies.CURSOR', () => {
      it(`should return a connection`, () =>
        request(app.getHttpServer())
          ...
          .expect(200)
          .then(({ body }) => {
            const { edges, pageInfo }: CursorConnectionType<TodoItemDTO> = body.data.todoItemsCursor
      ...
      it(`should allow querying`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          todoItemsCursor(filter: { id: { in: [1, 2, 3] } }) {
            ${cursorPageInfoField}
            ${edgeNodes(todoItemFields)}
          }
        }`
      ...
      it(`should allow sorting`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          todoItemsCursor(sorting: [{field: id, direction: DESC}]) {
            ${cursorPageInfoField}
            ${edgeNodes(todoItemFields)}
          }
        }`
      ...
      it(`should allow sorting (custom name)`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            // language=graphql
            query: `{
            todoItemsCursor(sorting: [{field: completed, direction: DESC},{field: id, direction: DESC}]) {
              ${cursorPageInfoField}
              ${edgeNodes(todoItemFields)}
            }
          }`
        ...
        it(`should allow paging with the 'first' field`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          todoItemsCursor(paging: {first: 2}) {
            ${cursorPageInfoField}
            ${edgeNodes(todoItemFields)}
          }
        }`
        ...
        it(`should allow paging with the 'first' field and 'after'`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          todoItemsCursor(paging: {first: 2, after: "YXJyYXljb25uZWN0aW9uOjE="}) {
            ${cursorPageInfoField}
            ${edgeNodes(todoItemFields)}
          }
        }`
